...
if(!isNull _unit && !("" in [_objectTypeToBuild,_buildingAvailableBoolString])) then {
    ...
    if(_currentAnimation <= _numberOfAnimations) then {
        ...
        // create object
        private _object = _objectTypeToBuild createVehicle _posASL;
        _object setDir _direction;
        _object setPosASL _posASL;
        // mission specific medical tent
        if (_objectTypeToBuild isEqualTo "CampEast_EP1") then {
            _object setVariable ["ace_medical_isMedicalFacility", true, true];
            private _grassCutter = "Land_ClutterCutter_large_F" createVehicle _posASL;
