...
if(isServer) then {
    {
        if(local _x) then {
            if(side _x == west) then {
                ...
                [_x] call Spec_fnc_loadout_aaf;
                _x addEventHandler ["Respawn", {(_this select 0) call Spec_fnc_loadout_aaf;}];
            } else {
...
if(hasInterface) then {
    [player] call Spec_fnc_loadout_aaf;
    player addEventHandler ["Respawn", {
        params ["_unit"];
        [_unit] call Spec_fnc_loadout_aaf;
    }];
};
true
