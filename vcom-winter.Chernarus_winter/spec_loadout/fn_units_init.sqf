...
if(isServer) then {
    {
        if(local _x) then {
            if(side _x == blufor) then {
                ...
                [_x] call Spec_fnc_loadout_rus_winter;
                _x addEventHandler ["Respawn", {(_this select 0) call Spec_fnc_loadout_rus_winter;}];
            } else {
...
if(hasInterface) then {
    [player] call Spec_fnc_loadout_rus_winter;
    player addEventHandler ["Respawn", {
        params ["_unit"];
        [_unit] call Spec_fnc_loadout_rus_winter;
    }];
};
true
