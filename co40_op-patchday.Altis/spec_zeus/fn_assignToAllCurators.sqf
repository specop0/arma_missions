		...
		private _scriptHandle = _this spawn {
			...
			if(_parameterCorrect) then {
				...
				if(!isNull (getAssignedCuratorLogic _player) ) then {
					[_player, ["Assign Units", {(getAssignedCuratorLogic (_this select 1)) addCuratorEditableObjects [allUnits, false];}, nil, 1.5, false, true, "", "true"] ] remoteExec ["addAction", _player] ;
				};
			};
		};
	} else {
