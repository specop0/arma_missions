...
comment "Missile Specialist (AA)";
private _aaWeapon = "BWA3_Fliegerfaust";
private _aaAmmo = ["BWA3_Fliegerfaust_Mag", "BWA3_Fliegerfaust_Mag"];
    ...
    if(_type == CLASS_MG) then {
        ...
        if(_type == CLASS_LMG) then {
            ...
            if(_type in [CLASS_TF, CLASS_GL, CLASS_FUNKER]) then {
                ...
                comment "AA Ammunition for AA Assistant";
                if(_type == CLASS_AA_ASSI) then {
                    {
                        [_unit,_x,ADD_TO_BACKPACK] call Spec_fnc_addItemToContainer;
                    } forEach _aaAmmo;
                };
